...
{	// check for errors
 	...
 	else // no errors
 	{
		...
		if ($_POST['user_name'] != 'admin') {$role = 'user';}
		else {$role = 'admin';} //make sure role is set
		//$image = $_FILES['user_image_url'];
		...
		$sql = "UPDATE 
		 ...
		 if(!$result)
		 {
			 echo "Unexpected Error when saving edit.";
			 //echo mysql_error();
		 }
		 else // return admin to profiles page
		 {
			...
			// GOTO index
			header("location:profiles.php?message=edit");
			 exit;
		 }
	}
}
...
{
	echo "Changes could not be saved because of missing data.";
}
